...
- `worx_vision_cloud.start_one_time_mowing` / translated service for one-time mowing with runtime, edge cutting and experimental zone selection
...
- current zone / legacy zone value or RTK map polygon containing the live mower position; RTK placeholder `0` is ignored
- RSSI
- rain delay
- rain remaining
...
- One-time mowing zones (experimental on Vision/RTK)
...
- `zones` - optional zone ID list, for example `[1, 2]`. Boundary-wire mowers support one selected zone. Vision/RTK firmware may ignore this field because the official app appears to use a different private cloud route. Leave it empty for default/all-zone mowing.
